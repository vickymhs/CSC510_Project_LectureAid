...
export default function CustomizedAccordions(props) {
  ...
  const addBookmarkToLocalStorage = (event, item) => {
    ...
    if (
      links.includes(link) &&
      question === linksInStorage[link].question &&
      simpleAnswer === linksInStorage[link].simple_answer
    ) {
      ...
      {rows.map((item, index) => (
        ...
        <Accordion key={"question" + index}>
          <AccordionSummary aria-controls={"question" + index + "-content"} id={"question" + index + "-header"}>
            ...
            <Button size="small" onClick={(e) => addBookmarkToLocalStorage(e, item)}>
              {getButtonIcon(item) ? (
                <BookmarkIcon className={classes.bookmarkedIcon} />
              ) : (
                <BookmarkIcon className={classes.unBookmarkedIcon} />
              )}
            </Button>
          </AccordionSummary>
          <AccordionDetails>
